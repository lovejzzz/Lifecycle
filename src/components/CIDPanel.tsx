...
import type { ProactiveSuggestion as _ProactiveSuggestion } from '@/lib/suggestions';
import { getAgent } from '@/lib/agents';
import type { CIDCard } from '@/lib/types';
import { relativeTime } from '@/lib/types';
import { renderMarkdown } from '@/lib/markdown';
import { exportAndDownload } from '@/lib/export';
import { formatCost } from '@/lib/cache';
  ...
  [
    ...
    },
    {
      ...
      hints: [
        ...
        {
          trigger: 'interpret override',
          label: 'interpret override <id> — AI-interpret an override',
        },
        {
          trigger: 'propagate override',
          label: 'propagate override <id> <scope> — Apply override (this-node|all-similar|global)',
        },
...
const QUICK_ACTIONS = [
  {
    ...
    icon: Wrench,
    prompt: 'Analyze the workflow and solve any problems you find',
  },
  {
    label: 'Propagate changes',
    icon: RefreshCw,
    prompt: 'Propagate all pending state changes to stale artifacts',
  },
  {
    label: 'Optimize workflow',
    icon: Zap,
    prompt: 'Analyze and optimize the current workflow graph',
  },
...
export default function CIDPanel() {
  const {
    ...
    isProcessing,
    nodes,
    propagateStale: _propagateStale,
    optimizeLayout,
    cidSolve,
    showImpactPreview,
    cidMode,
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      setInput(prompt);
      addToast(
        `${file.name} parsed: ${data.sections?.length ?? 0} sections, ~${data.tokenEstimate} tokens`,
        'success',
      );
    } catch (err) {
  ...
  const handleSend = () => {
    ...
    if (/^(?:compile|\/compile)\b/i.test(prompt)) {
      ...
      const format = formatMatch
        ? (formatMatch[1].replace('text', 'txt').replace('markdown', 'md') as 'md' | 'html' | 'txt')
        : 'md';
        ...
        setTimeout(
          () =>
            sendStreamingResponse(
              'Available templates: **Software Development**, **Content Pipeline**, **Incident Response**, **Product Launch**, **Chatbot**, **Course Design**, **Lesson Planning**, **Assignment Design**\n\nUse: `/template Software Development`',
            ),
          200,
        );
        return;
    ...
    const isExtendRequest =
      nodes.length > 0 &&
      /^(?:add|extend|expand|include|append|insert|also|plus|and also)\b/i.test(prompt);
    const isMakeGenerate = /^make\s+(?:a|an|me|new|my)\b/i.test(prompt);
    const isGenerateRequest =
      /^(?:build|create|generate|set up|design|start)\b/i.test(prompt) || isMakeGenerate;
    ...
    if (isArtifactCreate) {
      ...
      const typeMatch = prompt.match(
        /(?:create|generate|build|make|write)\s+(?:a\s+)?(?:new\s+)?([\w\s-]+)/i,
      );
      ...
      chatWithCID(
        `Extend the current workflow: ${prompt}. Add new nodes and connect them to existing nodes where appropriate. Return a workflow with ONLY the NEW nodes and edges to add. Use the existing node labels in edge references.`,
      );
      ...
      setTimeout(
        () => {
          ...
          if (aiEnabled && nodes.length > 0) {
            ...
            chatWithCID(
              `I just ran a structural fix on the workflow and found: ${result.message}. Analyze this workflow further — are there any gaps, missing connections, or improvements I should make? Be specific about which nodes. Do NOT return a workflow, just analysis.`,
            );
          } else {
      ...
      /^(?:health\s+detail|health\s+breakdown|detailed?\s+health|health\s+report)\s*$/i.test(prompt)
    ) {
      ...
      if (aiEnabled && nodes.length > 0) {
        // chatWithCID adds its own user message, so we don't add one here
        chatWithCID(
          `Give me a status report on this workflow. Analyze the health, identify any bottlenecks or gaps, and suggest the most impactful next step. Be concise and specific. Do NOT return a workflow, just your analysis.`,
        );
      } else {
      ...
      /^run\s+(?:everything|all|anything)\s+(?:that(?:'s| is)|which\s+is)\s+stale/i.test(prompt)
    ) {
      addMessage({ id: `msg-${Date.now()}`, role: 'user', content: prompt, timestamp: Date.now() });
      // Central Brain: use surgical sync when context exists
      if (
        hasContext &&
        /^(?:sync|sync all|sync stale|sync everything|resync|re-sync)\s*$/i.test(prompt)
      ) {
        setProcessing(true);
        useLifecycleStore
          .getState()
      ...
      if (sc > 0) {
        ...
        showImpactPreview();
        const _agent = getAgent(cidMode);
        sendStreamingResponse(
          `${sc} stale node${sc > 1 ? 's' : ''} found. Review the impact preview below and select which to regenerate.`,
        );
      } else {
        setTimeout(() => {
      ...
      dispatchCommand(prompt, () => {
        ...
        return count > 0
          ? `Done. Approved ${count} node${count > 1 ? 's' : ''} that were in review.`
          : 'No nodes currently in review status.';
      });
    } else if (/^(?:unlock\s+all|batch\s+unlock)\b/i.test(prompt)) {
      dispatchCommand(prompt, () => {
        const count = batchUpdateStatus('locked', 'active');
        return count > 0
          ? `Done. Unlocked ${count} node${count > 1 ? 's' : ''}.`
          : 'No locked nodes found.';
      });
    } else if (
      /^(?:activate\s+all|batch\s+activate)\b/i.test(prompt) ||
      /^(?:mark|set)\s+(?:all|every\w*)\s+(?:\w+\s+)?(?:as|to)\s+active\s*$/i.test(prompt)
    ) {
      dispatchCommand(prompt, () => {
        const count = batchUpdateStatus('stale', 'active');
        return count > 0
          ? `Done. Activated ${count} stale node${count > 1 ? 's' : ''}.`
      ...
      /^(?:remove|break|cut)\s+(?:the\s+)?(?:connection|link|edge)\s+(?:between|from)\s+/i.test(
        prompt,
      )
    ) {
      dispatchCommand(prompt, () => disconnectByName(prompt).message, 400, undefined, true);
      ...
      if (delPreview) {
        ...
        if (target) {
          ...
          if (
            connCount > 0 &&
            !window.confirm(
              `Delete "${target.data.label}"? This will remove ${connCount} connection${connCount > 1 ? 's' : ''}.`,
            )
          ) {
            addMessage({
              id: `msg-${Date.now()}`,
      ...
      /^(?:show|find|list)\s+(?:me\s+)?(?:what(?:'s| is)\s+)?stale/i.test(prompt) ||
      /^(?:show|find|list)\s+stale/i.test(prompt) ||
      /^(?:what|which)\s+nodes?\s+(?:are|is)\s+stale/i.test(prompt) ||
      /^how\s+many\s+(?:nodes?\s+)?(?:are\s+)?stale/i.test(prompt)
    ) {
      ...
      /^(?:focus|select|show|go to|find|zoom)\s+(?:on\s+)?["']?.+["']?\s*$/i.test(prompt) &&
      !/^show\s+(?:me\s+)?(?:your\s+)?(?:understanding|context|source|overrides?)\s*$/i.test(prompt)
    ) {
      addMessage({ id: `msg-${Date.now()}`, role: 'user', content: prompt, timestamp: Date.now() });
      const nameMatch = prompt.match(
        /(?:focus|select|show|go to|find|zoom)\s+(?:on\s+)?["']?(.+?)["']?\s*$/i,
      );
      if (nameMatch) {
        ...
        if (found) {
          ...
          setTimeout(
            () =>
              sendStreamingResponse(
                `Focused on **${found.data.label}** (${found.data.category}, ${found.data.status}).`,
              ),
            200,
          );
        } else {
          ...
          setTimeout(
            () =>
              sendStreamingResponse(
                `No node matching "${nameMatch[1]}". Available: ${nodes.map((n) => n.data.label).join(', ')}.`,
              ),
            200,
          );
        }
      ...
      if (dupMatch) {
        ...
        if (found) {
          ...
          setTimeout(
            () =>
              sendStreamingResponse(
                `Duplicated **${found.data.label}**. The copy has been placed nearby.`,
              ),
            300,
          );
        } else {
          ...
          setTimeout(
            () =>
              sendStreamingResponse(
                `No node matching "${dupMatch[1]}". Available: ${nodes.map((n) => n.data.label).join(', ')}.`,
              ),
            200,
          );
        }
      ...
      /^(?:add|new)\s+\w+\s+(?:called|named|:)\s+/i.test(prompt) ||
      /^(?:add|new)\s+\w+\s+["'].+["']/i.test(prompt)
    ) {
      dispatchCommand(prompt, () => addNodeByName(prompt).message, 400, undefined, true);
    } else if (
      /^(?:set|mark|change)\s+.+\s+(?:to|as|→)\s+\w+\s*$/i.test(prompt) ||
      /^lock\s+["']?.+["']?\s*$/i.test(prompt) ||
      /^unlock\s+["']?.+["']?\s*$/i.test(prompt)
    ) {
      ...
      dispatchCommand(prompt, () => {
        const nameMatch = prompt.match(
          /(?:download|export\s+node)\s+["']?(.+?)["']?\s*(?:as\s+(\w+))?\s*$/i,
        );
        if (!nameMatch) return 'Usage: download <node name> [as md|html|txt]';
        const nodeName = nameMatch[1];
        const format = (nameMatch[2]
          ?.toLowerCase()
          .replace('text', 'txt')
          .replace('markdown', 'md') || 'md') as 'md' | 'html' | 'txt';
      ...
      dispatchCommand(
        ...
        () => {
          if (history.length > 0) {
            undo();
            return 'Done. Reverted to previous state.';
          }
      ...
      dispatchCommand(
        ...
        () => {
          if (future.length > 0) {
            redo();
            return 'Done. Reapplied the last undone change.';
          }
      ...
      if (
        staleNodes.length > 0 &&
        !window.confirm(
          `Remove ${staleNodes.length} stale node${staleNodes.length > 1 ? 's' : ''}: ${staleNodes.map((n) => n.data.label).join(', ')}?`,
        )
      ) {
        addMessage({
          id: `msg-${Date.now()}`,
      ...
      /^(?:orphan|isolat|unconnected)\w*\s*$/i.test(prompt) ||
      /^(?:show|find|list)\s+(?:me\s+)?(?:the\s+)?(?:orphan|unconnected|isolat)\w*/i.test(prompt)
    ) {
      ...
      /^(?:deps|dependencies|depend|upstream|downstream|chain)\s+/i.test(prompt) ||
      /^what(?:'s| is)\s+(?:blocking|preventing|stopping)\s+.+\s+(?:from|to)\s+/i.test(prompt) ||
      /^what(?:'s|\s+(?:is|are))?\s+(?:depend(?:s|ent|ing)?|downstream|upstream)\s+(?:on|of|from)\s+/i.test(
        prompt,
      ) ||
      /^what\s+depends\s+on\s+/i.test(prompt)
    ) {
      ...
      /^(?:pre\s*flight|flight\s*check|dry\s*run|plan\s+run|execution\s+plan)\s*$/i.test(prompt) ||
      ...
      /^(?:diff\s+(?:last|prev(?:ious)?)|compare\s+(?:run|execution)s?)\s*$/i.test(prompt)
    ) {
      dispatchCommand(prompt, () => diffLastRun());
    } else if (
      /^(?:refresh|update|regenerate)\s+(?:the\s+)?["']?(.+?)["']?\s*$/i.test(prompt) &&
      !/^(?:update|change|edit|modify)\s+(?:the\s+)?(?:source|input|original|context)\b/i.test(
        prompt,
      )
    ) {
      // "refresh the quiz bank", "update the rubric", "regenerate study guide" → execute specific node
      const refreshMatch = prompt.match(
        /^(?:refresh|update|regenerate)\s+(?:the\s+)?["']?(.+?)["']?\s*$/i,
      );
      if (refreshMatch) {
        ...
        if (target) {
          ...
          setTimeout(
            () =>
              sendStreamingResponse(
                `No node matching "${refreshMatch[1]}". Available: ${nodes.map((n) => n.data.label).join(', ')}.`,
              ),
            200,
          );
        }
      ...
      if (runMatch) {
        ...
        if (target) {
          ...
          setTimeout(
            () =>
              sendStreamingResponse(
                `No node matching "${runMatch[1]}". Available: ${nodes.map((n) => n.data.label).join(', ')}.`,
              ),
            200,
          );
        }
      ...
      if (aiEnabled && nodes.length > 0) {
        // chatWithCID adds its own user message, so we don't add one here
        chatWithCID(
          `Explain this workflow step by step as a narrative. Walk through each node and how they connect. Be clear and engaging. Do NOT return a workflow, just the explanation.`,
        );
      } else {
      ...
      /^(?:why|reason|purpose)\s+/i.test(prompt) &&
      !/^why\s+(?:is|does|do|are|was|were|can|should|would|has|have|did)\b/i.test(prompt)
    ) {
      dispatchCommand(prompt, () => whyNode(prompt));
    } else if (
      /^(?:relabel|re-label|fix\s+labels?|infer\s+labels?)\s*(?:all|edges?)?\s*$/i.test(prompt)
    ) {
      ...
      dispatchCommand(prompt, () => {
        ...
        const bar =
          '█'.repeat(Math.floor(p.percent / 5)) + '░'.repeat(20 - Math.floor(p.percent / 5));
      ...
      /^(?:plan|execution\s*plan|steps|order)\s*$/i.test(prompt) ||
      /^what(?:'s|\s+is)\s+the\s+(?:execution\s+)?order/i.test(prompt)
    ) {
      ...
      const contentMatch = prompt.match(
        /^(?:ingest|feed|analyze|here(?:'s| is) (?:my|the|some)|take this|source(?:\s*material)?:?)\s*([\s\S]*)/i,
      );
      ...
      if (understanding) {
        ...
        dispatchCommand(
          prompt,
          () =>
            'No source material ingested yet. Paste your content or say "ingest: <your content>" to get started.',
        );
      }
    } else if (
      /^(?:create|generate|build|make|write)\s+(?:a\s+)?(?:new\s+)?(?:blog[\s-]?post|email|social[\s-]?(?:thread|post|media)|twitter[\s-]?thread|x[\s-]?thread|ad[\s-]?copy|press[\s-]?release|landing[\s-]?page|newsletter|product[\s-]?description|linkedin[\s-]?post|ph[\s-]?(?:tagline|copy)|pitch|summary|brief|article|copy)/i.test(
      ...
      if (syncMatch) {
        ...
        if (target) {
          ...
          useLifecycleStore
            .getState()
            .syncArtifact(target.id)
            .then((diff) => {
              if (diff) {
                sendStreamingResponse(
                  `Synced **${target.data.label}**: ${diff.changes.length} change${diff.changes.length !== 1 ? 's' : ''} applied.`,
                );
              } else {
                sendStreamingResponse(
                  `**${target.data.label}** is already current or has no artifact contract.`,
                );
              }
            });
        } else {
      ...
      /^(?:diff|preview sync|what(?:'s| would) (?:change|sync)|stale artifacts)\s*$/i.test(prompt)
    ) {
      ...
      /^(?:overrides?|show overrides?|list overrides?|my (?:edits|changes|overrides))\s*$/i.test(
        prompt,
      )
    ) {
      const ctx = useLifecycleStore.getState().centralContext;
      dispatchCommand(prompt, () => {
        if (!ctx || ctx.overrides.length === 0)
          return 'No overrides recorded. Overrides are tracked when you manually edit CID-managed artifacts.';
      ...
      if (match) {
        ...
        setTimeout(async () => {
          ...
          if (interpretation) {
            ...
            sendStreamingResponse(
              `Could not interpret override \`${overrideId}\`. It may not exist or has no context.`,
            );
          }
        }, 300);
      }
      ...
      const match = prompt.match(
        /^propagate\s+override\s+(\S+)\s+(this-node|all-similar|global)\s*$/i,
      );
      if (match) {
        const overrideId = match[1];
      ...
      const newContent = prompt
        .replace(
          /^(?:update|change|edit|modify)\s+(?:the\s+)?(?:source|input|original|context)\s*/i,
          '',
        )
        .trim();
      if (newContent) {
        ...
        setTimeout(
          () =>
            sendStreamingResponse(
              'Provide the updated source content after the command. Example: `update source: <new content>`',
            ),
          200,
        );
      }
  ...
  const handleRefinementClick = (suggestionId: string) => {
    const refinement =
      typeof window !== 'undefined'
        ? ((window as unknown as Record<string, unknown>).__lifecycleRefinement as
            | {
                parsed: {
    ...
    if (prefix === 'refine-clean' && parsed.cleanedContent) {
      ...
      const edgesBetweenExisting =
        parsed.suggestedEdges?.filter(
          (se) =>
            nodes.some((n) => n.data.label === se.from) &&
            nodes.some((n) => n.data.label === se.to),
        ) || [];
  ...
  const showCards =
    agent.interviewEnabled && poirotContext.phase === 'interviewing' && lastCardMessage?.cards;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="group absolute top-0 bottom-0 left-0 z-10 w-1 cursor-col-resize transition-colors hover:bg-emerald-500/20 active:bg-emerald-500/30"
        title="Drag to resize"
      >
        <div className="absolute top-1/2 left-0 h-8 w-1 -translate-y-1/2 rounded-full bg-white/0 transition-colors group-hover:bg-white/20" />
      </div>
      {/* Header */}
      <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4">
        <div className="flex items-center gap-3">
          <div className="relative" data-model-picker>
            <button
              ...
              className={`flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg transition-all hover:ring-2 ${
                isAmber
                  ? 'bg-amber-500/10 hover:ring-amber-500/30'
                  : 'bg-emerald-500/10 hover:ring-emerald-500/30'
              }`}
            ...
            {showModelPicker && (
              <div className="animate-in fade-in slide-in-from-top-2 absolute top-10 left-0 z-50 w-56 rounded-xl border border-white/[0.1] bg-[#0e0e18]/98 py-1.5 shadow-2xl backdrop-blur-xl duration-150">
                <div className="px-3 py-1.5 text-[10px] font-medium tracking-wider text-white/40 uppercase">
                  AI Model
                </div>
                {AI_MODELS.map((m) => (
                  <button
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="text-[11px] font-medium text-white/80">{m.label}</div>
                      <div className="text-[10px] text-white/40">{m.desc}</div>
                    </div>
                    {cidAIModel === m.id && (
                      <Check
                        size={12}
                        className={isAmber ? 'text-amber-400' : 'text-emerald-400'}
                      />
                    )}
                  </button>
                ))}
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 text-[10px] tracking-wider text-white/45 uppercase">
              {agent.subtitle}
              <span
                className={`inline-flex items-center gap-0.5 rounded px-1 py-px text-[8px] font-medium ${
                  aiEnabled
              ...
              >
                ...
                {AI_MODELS.find((m) => m.id === cidAIModel)
                  ?.label.split(' ')
                  .slice(-2)
                  .join(' ') || 'Sonnet 4'}
              </span>
              {useLifecycleStore.getState().hasContext() && (
                <span
                  className="inline-flex items-center gap-0.5 rounded bg-cyan-500/15 px-1 py-px text-[8px] font-medium text-cyan-400/80"
                  title="Source material ingested"
                >
                  🧠 Context
                </span>
              )}
              {useLifecycleStore.getState()._sessionCost.callCount > 0 && (
                <span
                  className="inline-flex items-center gap-0.5 rounded bg-amber-500/15 px-1 py-px text-[8px] font-medium text-amber-400/80"
                  title={`${useLifecycleStore.getState()._sessionCost.callCount} calls, ${useLifecycleStore.getState()._sessionCost.totalTokens.toLocaleString()} tokens`}
                >
                  {formatCost(useLifecycleStore.getState()._sessionCost.estimatedCostUSD)}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button
            ...
            title="Clear chat"
            className="flex h-7 w-7 items-center justify-center rounded-lg text-white/30 transition-colors hover:bg-rose-500/10 hover:text-rose-400/60"
          >
            <Trash2 size={12} />
          </button>
          {/* Close */}
      ...
      {statsData && (
        <div className="flex items-center gap-3 border-b border-white/[0.04] px-5 py-1.5 text-[10px] text-white/40">
          ...
          {statsData.progress.total > 0 && (
            <>
              <span className="text-white/30">·</span>
              <span className="text-cyan-400/50">{statsData.progress.percent}%</span>
            </>
          )}
          {statsData.stale > 0 && (
            <>
              <span className="text-white/30">·</span>
              <span className="text-amber-400/50">{statsData.stale} stale</span>
            </>
          )}
          {statsData.orphans > 0 && (
            <>
              ...
              <span className="text-rose-400/50">
                {statsData.orphans} orphan{statsData.orphans > 1 ? 's' : ''}
              </span>
            </>
          )}
        </div>
        ...
        (() => {
          ...
          return (
            <details className="border-b border-white/[0.04]">
              <summary className="flex cursor-pointer items-center gap-2 px-5 py-2 text-[10px] text-cyan-400/60 transition-colors select-none hover:bg-white/[0.02]">
                <Brain size={11} className="flex-shrink-0" />
                <span className="font-medium tracking-wider uppercase">Context</span>
              ...
              <div className="space-y-1.5 px-5 pb-2.5 text-[9.5px]">
                {ctx.source.title && (
                  <p className="font-medium text-white/60">{ctx.source.title}</p>
                )}
                <p className="leading-relaxed text-white/35">{understanding.summary}</p>
                ...
                <div className="mt-1 flex items-center gap-3 text-[8px] text-white/20">
      ...
      <div
        ...
        className="scrollbar-thin relative flex-1 space-y-4 overflow-y-auto px-4 py-4"
        onScroll={() => {
      ...
      >
        ...
        {pinnedMessageIds.size > 0 && (
          <div
            className={`mb-3 space-y-1.5 rounded-lg border px-3 py-2 ${
              isAmber
          ...
          >
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <span
                className={`text-[9px] font-medium tracking-wider uppercase ${isAmber ? 'text-amber-400/40' : 'text-emerald-400/40'}`}
              >
                Pinned
              </span>
            </div>
            {messages
              ...
              .map((msg) => (
                <div key={`pin-${msg.id}`} className="group/pin flex items-start gap-1.5">
                  <div className="line-clamp-2 flex-1 text-[10px] leading-snug text-white/50">
                    {msg.content.slice(0, 120)}
                    {msg.content.length > 120 ? '...' : ''}
                  </div>
                  <button
                    onClick={() => togglePinMessage(msg.id)}
                    className="mt-0.5 flex-shrink-0 text-white/20 opacity-0 transition-all group-hover/pin:opacity-100 hover:text-white/50"
                  >
                    <X size={8} />
                  </button>
                </div>
        ...
        {messages.length <= 1 && nodes.length === 0 && (
          <div className="mb-4 space-y-2">
            <p
              className={`text-[11px] ${isAmber ? 'text-amber-400/50' : 'text-emerald-400/50'} font-medium`}
            >
              Quick Start
            </p>
            {[
              ...
              <button
                ...
                className={`w-full rounded-lg border px-3 py-2 text-left text-[11px] transition-all ${
                  isAmber
                    ? 'border-amber-500/10 text-white/40 hover:border-amber-500/20 hover:bg-amber-500/[0.06] hover:text-amber-300/80'
                    : 'border-emerald-500/10 text-white/40 hover:border-emerald-500/20 hover:bg-emerald-500/[0.06] hover:text-emerald-300/80'
                }`}
              >
                {template}
              </button>
        ...
        {messages.map((msg, msgIdx) => {
          ...
          const isGrouped =
            msg.role === 'cid' &&
            prevMsg?.role === 'cid' &&
            msg.timestamp - prevMsg.timestamp < 2000;
          return (
            <div
              key={`${msg.id}-${msgIdx}`}
              ...
              <div className="relative max-w-[90%]">
                <div
                  className={`rounded-xl px-3.5 py-2.5 text-[12.5px] leading-relaxed ${
                    msg.role === 'user'
                      ? 'rounded-br-sm bg-white/[0.08] text-white/80'
                      : isAmber
                        ? `border border-amber-500/[0.12] bg-amber-500/[0.07] text-white/70 ${isGrouped ? 'rounded-tl-sm rounded-bl-sm border-t-0' : 'rounded-bl-sm'}`
                        : `border border-emerald-500/[0.12] bg-emerald-500/[0.07] text-white/70 ${isGrouped ? 'rounded-tl-sm rounded-bl-sm border-t-0' : 'rounded-bl-sm'}`
                  }`}
                >
                  {msg.role === 'cid' && msg.action === 'thinking' && (
                    <div
                      className={`mb-1 flex items-center gap-2 ${isAmber ? 'text-amber-400/70' : 'text-emerald-400/70'}`}
                    >
                  ...
                  {msg.role === 'cid' && msg.action === 'building' && (
                    <div
                      className={`flex items-center gap-2 ${isAmber ? 'text-amber-400/70' : 'text-emerald-400/70'}`}
                    >
                      <Loader2 size={11} className="animate-spin" />
                  ...
                  {msg.role === 'cid' && msg.action === 'investigating' && (
                    <div className="mb-1 flex items-center gap-2 text-amber-400/70">
                      <Search size={11} className="animate-pulse" />
                      <span className="text-[10px] font-medium tracking-wider uppercase">
                        {agent.investigatingLabel}
                      </span>
                    </div>
                  )}
                  {msg.role === 'cid' && msg.action !== 'building'
                    ? renderMarkdown(msg.content, nodeNameMap, (id) => selectNode(id))
                    : msg.action !== 'building'
                      ? msg.content
                      : null}
                  {msg.role === 'cid' && msg.content === '' && !msg.action && (
                    <span
                      className={`inline-block h-3.5 w-1.5 animate-pulse rounded-sm ${
                ...
                <div
                  className={`mt-0.5 text-[9px] text-white/0 transition-colors group-hover/msg:text-white/20 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}
                >
                    ...
                    {msg.suggestions.map((s) => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-lg border px-2.5 py-1 text-[10px] transition-all hover:scale-[1.03] ${
                            isRefinement
                              ? 'border-violet-500/20 bg-violet-500/[0.06] text-violet-400/70 hover:bg-violet-500/[0.12]'
                              : isAction
                                ? 'border-cyan-500/20 bg-cyan-500/[0.06] text-cyan-400/70 hover:bg-cyan-500/[0.12]'
                                : isAmber
                                  ? 'border-amber-500/20 bg-amber-500/[0.06] text-amber-400/70 hover:bg-amber-500/[0.12]'
                                  : 'border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-400/70 hover:bg-emerald-500/[0.12]'
                          }`}
                        >
                          {displayLabel}
                        </button>
                ...
                {msg.role === 'cid' && msg.content && (
                  <button
                    ...
                    className={`absolute top-1 -right-6 flex h-5 w-5 items-center justify-center rounded opacity-0 transition-all group-hover/msg:opacity-100 ${
                ...
                {msg.role === 'user' && !isProcessing && (
                  <button
                    ...
                    title="Edit & resend"
                    className="absolute top-1/2 -left-6 flex h-5 w-5 -translate-y-1/2 items-center justify-center rounded text-white/20 opacity-0 transition-all group-hover/msg:opacity-100 hover:text-white/50"
                  >
                    <Pencil size={10} />
                  </button>
                )}
          ...
          >
            {lastCardMessage!.cards!.map((card) => (
              <button
                ...
                className="group rounded-xl border border-amber-500/[0.15] bg-amber-500/[0.04] p-3 text-left transition-all hover:border-amber-500/[0.3] hover:bg-amber-500/[0.1] disabled:opacity-30"
              >
                <div className="mb-1 flex items-center justify-between">
                  <span className="text-[11px] font-semibold text-amber-300/90">{card.label}</span>
                  <ChevronRight
                    size={10}
                    className="text-amber-400/30 transition-colors group-hover:text-amber-400/60"
                  />
                </div>
                {card.description && (
                  <span className="text-[9.5px] leading-snug text-white/45">
                    {card.description}
                  </span>
                )}
              </button>
        ...
        {isProcessing &&
          !messages.some(
            (m) =>
              m.action === 'thinking' || m.action === 'investigating' || m.action === 'building',
          ) &&
          !(
            messages.length > 0 &&
            messages[messages.length - 1].role === 'cid' &&
            messages[messages.length - 1].content
          ) && (
            <div className="flex justify-start">
              <div
                className={`rounded-xl rounded-bl-sm px-3.5 py-2 text-[12px] ${
                  isAmber
                    ? 'border border-amber-500/[0.12] bg-amber-500/[0.07]'
                    : 'border border-emerald-500/[0.12] bg-emerald-500/[0.07]'
                }`}
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`text-[10px] font-medium ${isAmber ? 'text-amber-400/70' : 'text-emerald-400/70'}`}
                  >
                    {isAmber ? 'Poirot' : 'Rowan'}
                  </span>
                  <div className="flex items-center gap-1">
                    <span
                      className={`h-1.5 w-1.5 animate-bounce rounded-full ${isAmber ? 'bg-amber-400/60' : 'bg-emerald-400/60'}`}
          ...
          <button
            ...
            className={`rounded-full border px-3 py-1 text-[10px] transition-colors ${
              isAmber
                ...
                : 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400/70 hover:bg-emerald-500/20'
            }`}
          >
            New messages below
          </button>
          ...
          <div className="scrollbar-none flex gap-1.5 overflow-x-auto pb-1">
            {QUICK_ACTIONS.map((action) => (
              <button
                ...
                disabled={isProcessing}
                className={`flex items-center gap-1.5 rounded-lg border border-white/[0.06] bg-white/[0.04] px-2.5 py-1.5 text-[10px] whitespace-nowrap text-white/45 transition-all hover:border-white/[0.1] hover:bg-white/[0.07] hover:text-white/70 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-white/[0.04] disabled:hover:text-white/45`}
              >
                <action.icon size={10} />
                {action.label}
              </button>
      ...
      <div className="relative px-4 pt-2 pb-4">
        ...
        {matchingHints.length > 0 && (
          <div className="absolute right-4 bottom-full left-4 z-10 mb-1 max-h-[300px] overflow-hidden overflow-y-auto rounded-lg border border-white/[0.08] bg-[#0e0e18]/95 shadow-xl backdrop-blur-xl">
            {matchingHints.map((h, i) => (
              <div key={h.trigger + i}>
                {'section' in h && h.section && (
                  <div className="border-t border-white/[0.04] px-3 py-1 text-[9px] font-medium tracking-wider text-white/35 uppercase first:border-t-0">
                    {h.section}
                  </div>
                )}
                <button
                  ...
                  className="w-full px-3 py-1.5 text-left text-[11px] text-white/50 transition-colors hover:bg-white/[0.05] hover:text-white/80"
                >
                  {h.label}
                </button>
              </div>
        ...
        {uploadedFile && (
          <div className="mb-1 flex items-center gap-2 rounded-lg border border-cyan-500/20 bg-cyan-500/10 px-3 py-1.5 text-[11px]">
            <FileText size={12} className="shrink-0 text-cyan-400" />
            <span className="truncate text-cyan-300">{uploadedFile.name}</span>
            <span className="shrink-0 text-cyan-500/60">
              {uploadedFile.sections}s ~{uploadedFile.tokens}t
            </span>
            <button
              onClick={() => setUploadedFile(null)}
              className="ml-auto text-cyan-500/40 transition-colors hover:text-cyan-300"
            >
              <XCircle size={12} />
            </button>
          </div>
        ...
        <div
          className={`flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.04] px-3.5 py-2.5 transition-colors ${
        ...
        >
          ...
          <button
            ...
            className="flex h-7 w-7 items-center justify-center rounded-lg text-white/30 transition-colors hover:bg-white/[0.06] hover:text-white/60 disabled:opacity-20"
          >
          ...
          <input
            ...
            placeholder={
              poirotContext.phase === 'interviewing'
                ? agent.placeholderInterviewing
                : agent.placeholder
            }
            disabled={isProcessing}
            className="flex-1 bg-transparent text-[12.5px] text-white/80 placeholder-white/30 outline-none disabled:cursor-not-allowed disabled:opacity-40"
          />
          {isProcessing ? (
            <button
              onClick={() => {
              ...
              title="Stop"
              className="flex h-7 w-7 items-center justify-center rounded-lg bg-rose-500/20 text-rose-400 transition-colors hover:bg-rose-500/30"
            >
