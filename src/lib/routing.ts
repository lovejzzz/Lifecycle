...
interface PatternEntry { test: PatternTest; route: CommandRoute; confidence: ConfidenceLevel }
...
addPattern((s) => /^\/clear\s*$/i.test(s) || /^\/new\s*$/i.test(s) || /^\/export\s*$/i.test(s) || /^\/mode\s*$/i.test(s), 'template', 'high');
...
addPattern((s) => /^(?:add|attach|assign)\s+(?:a\s+)?(?:\w+\s+)?tool\s+(?:to|on)\s+/i.test(s), 'add-tool', 'high');
// Show tools: "show tools", "list tools", "tools"
addPattern((s) => /^(?:show|list)\s+(?:(?:available|all)\s+)?tools?\s*$/i.test(s) || /^tools?\s*$/i.test(s), 'show-tools', 'high');
// Set edge condition: "set condition on edge from X to Y", "add condition for Rubric→Review"
addPattern((s) => /^(?:set|add|configure)\s+(?:a\s+)?(?:condition|guard|filter)\s+(?:on|for|to)\s+/i.test(s), 'set-condition', 'high');
// Configure retry: "configure retry for Node X", "set up retries on Quiz Bank"
addPattern((s) => /^(?:configure|set\s+up|enable)\s+(?:retry|retries|backoff|fallback)\s+(?:for|on)\s+/i.test(s) || /^(?:retry\s+(?:policy|config(?:ure)?)|set\s+\d+\s+retr(?:y|ies))\s+/i.test(s), 'configure-retry', 'high');

// Extend vs generate
addPattern((s, hw) => hw && /^(?:add|extend|expand|include|append|insert|also|plus|and also)\b/i.test(s), 'extend', 'high');
addPattern((s) => /^(?:build|create|generate|set up|design|start)\b/i.test(s) || /^make\s+(?:a|an|me|new|my)\b/i.test(s), 'generate', 'high');

// Solve
addPattern((s) => /^(?:solve|fix|diagnose?|heal|repair)\b/i.test(s), 'solve', 'high');

// Health detail (MUST come before generic status)
addPattern((s) => /^(?:health\s+detail|health\s+breakdown|detailed?\s+health|health\s+report)\s*$/i.test(s), 'health-detail', 'high');
...
addPattern((s) => /^run\s+(?:everything|all|anything)\s+(?:that(?:'s| is)|which\s+is)\s+stale/i.test(s), 'propagate', 'high');
...
addPattern((s) => /^(?:mark|set)\s+(?:all|every\w*)\s+(?:\w+\s+)?(?:as|to)\s+(?:approved|done)\s*$/i.test(s), 'approve-all', 'high');
addPattern((s) => /^(?:unlock\s+all|batch\s+unlock)\b/i.test(s), 'unlock-all', 'high');
addPattern((s) => /^(?:mark|set)\s+(?:all|every\w*)\s+(?:\w+\s+)?(?:as|to)\s+unlocked\s*$/i.test(s), 'unlock-all', 'high');
addPattern((s) => /^(?:activate\s+all|batch\s+activate)\b/i.test(s), 'activate-all', 'high');
addPattern((s) => /^(?:mark|set)\s+(?:all|every\w*)\s+(?:\w+\s+)?(?:as|to)\s+active\s*$/i.test(s), 'activate-all', 'high');

// Connect / disconnect (MUST come before delete)
addPattern((s) => /^(?:connect|link|wire|attach)\s+.+\s+(?:to|with|→|->)\s+/i.test(s), 'connect', 'high');
addPattern((s) => /^(?:disconnect|unlink|unwire|detach)\s+.+\s+(?:from|and|→|->)\s+/i.test(s), 'disconnect', 'high');
addPattern((s) => /^(?:remove|break|cut)\s+(?:the\s+)?(?:connection|link|edge)\s+(?:between|from)\s+/i.test(s), 'disconnect', 'high');

// Delete
addPattern((s) => /^(?:delete|remove|drop|destroy)\s+.+/i.test(s), 'delete', 'high');

// Rename
addPattern((s) => /^(?:rename|change name|relabel)\s+.+\s+(?:to|as|→|->)\s+/i.test(s), 'rename', 'high');

// Show stale (MUST come before generic focus/show)
addPattern((s) => /^(?:show|find|list)\s+(?:me\s+)?(?:what(?:'s| is)\s+)?stale/i.test(s), 'show-stale', 'medium');
...
addPattern((s) => /^(?:show|find|what(?:'s| is| are))\s+(?:me\s+)?(?:the\s+)?(?:critical\s*path|longest\s*chain)/i.test(s), 'critical-path', 'medium');
addPattern((s) => /^(?:show|find|what(?:'s| is| are))\s+(?:me\s+)?(?:the\s+)?(?:bottleneck|chokepoint|hub|spof)/i.test(s), 'bottlenecks', 'medium');
addPattern((s) => /^(?:show|find|list)\s+(?:me\s+)?(?:the\s+)?(?:orphan|unconnected|isolat)\w*/i.test(s), 'orphans', 'medium');

// Focus / select / show <node>
addPattern((s) => /^(?:focus|select|show|go to|find|zoom)\s+(?:on\s+)?["']?.+["']?\s*$/i.test(s), 'focus', 'medium');

// Clone workflow (MUST come before duplicate)
addPattern((s) => /^(?:clone|duplicate)\s+(?:workflow|graph|project|all)\s*$/i.test(s), 'clone-workflow', 'high');

// Duplicate
addPattern((s) => /^(?:duplicate|clone|copy)\s+["']?.+["']?\s*$/i.test(s), 'duplicate', 'medium');

// Add node by name
addPattern((s) => /^(?:add|new)\s+\w+\s+(?:called|named|:)\s+/i.test(s) || /^(?:add|new)\s+\w+\s+["'].+["']/i.test(s), 'add-node', 'high');

// Set status / lock / unlock
addPattern((s) => /^(?:set|mark|change)\s+.+\s+(?:to|as|→)\s+\w+\s*$/i.test(s) || /^lock\s+["']?.+["']?\s*$/i.test(s) || /^unlock\s+["']?.+["']?\s*$/i.test(s), 'set-status', 'medium');

// List
addPattern((s) => /^(?:list|show|inventory)\s+/i.test(s), 'list', 'medium');

// Describe
addPattern((s) => /^(?:describe|annotate|document)\s+.+\s+(?:as:?|:)\s+/i.test(s), 'describe', 'high');
...
// Group
addPattern((s) => /^(?:group|cluster|organize)\s*(?:by\s*)?(?:category|type)?\s*$/i.test(s), 'group', 'high');
...
// Merge
addPattern((s) => /^(?:merge|combine|fuse)\s+.+\s+(?:and|with|into|&)\s+/i.test(s), 'merge', 'high');

// Deps
addPattern((s) => /^(?:deps|dependencies|depend|upstream|downstream|chain)\s+/i.test(s), 'deps', 'medium');
addPattern((s) => /^what(?:'s| is)\s+(?:blocking|preventing|stopping)\s+.+\s+(?:from|to)\s+/i.test(s), 'deps', 'medium');
addPattern((s) => /^what(?:'s|\s+(?:is|are))?\s+(?:depend(?:s|ent|ing)?|downstream|upstream)\s+(?:on|of|from)\s+/i.test(s), 'deps', 'medium');
...
addPattern((s) => /^(?:load|use)\s+template\s+["']?(.+?)["']?\s*$/i.test(s), 'load-template', 'high');
...
// Critical path
addPattern((s) => /^(?:critical\s*path|longest\s*chain|bottleneck)\s*$/i.test(s), 'critical-path', 'high');

// Isolate
addPattern((s) => /^(?:isolate|subgraph|neighborhood|neighbours?)\s+/i.test(s), 'isolate', 'medium');

// Summarize
addPattern((s) => /^(?:summarize|summary|executive|brief|overview)(?:\s+(?:the\s+)?(?:workflow|graph|project|all))?\s*$/i.test(s), 'summarize', 'high');

// Validate
addPattern((s) => /^(?:validate|integrity|check|audit)(?:\s+(?:the\s+)?(?:workflow|graph|project|all))?\s*$/i.test(s), 'validate', 'high');

// What if
addPattern((s) => /^(?:what\s*if|impact|without)\b/i.test(s), 'what-if', 'high');

// Preflight
addPattern((s) => /^(?:pre\s*flight|flight\s*check|dry\s*run|plan\s+run|execution\s+plan)\b/i.test(s), 'preflight', 'high');
addPattern((s) => /^(?:which|what)\s+nodes?\s+(?:are|is)\s+(?:ready|able|eligible)\s+(?:to\s+)?(?:run|execute)/i.test(s), 'preflight', 'medium');

// Retry failed
addPattern((s) => /^(?:retry|rerun|re-run)\s+(?:failed|errors?|skipped)\s*$/i.test(s), 'retry-failed', 'high');

// Clear results
addPattern((s) => /^(?:clear|reset)\s+(?:results?|execution|output)\s*$/i.test(s), 'clear-results', 'high');

// Diff last run
addPattern((s) => /^(?:diff\s+(?:last|prev(?:ious)?)|compare\s+(?:run|execution)s?)\s*$/i.test(s), 'diff-last-run', 'high');

// Refresh / update / regenerate specific node (MUST come before run-workflow/run-node)
addPattern((s) => /^(?:refresh|update|regenerate)\s+(?:the\s+)?["']?(.+?)["']?\s*$/i.test(s), 'refresh-node', 'medium');

// Run workflow
addPattern((s) => /^(?:run|execute|start)\s+(?:workflow|all|pipeline|everything)\s*$/i.test(s), 'run-workflow', 'high');
...
addPattern((s) => /^(?:why|reason|purpose)\s+/i.test(s) && !/^why\s+(?:is|does|do|are|was|were|can|should|would|has|have|did)\b/i.test(s), 'why', 'medium');

// Relabel
addPattern((s) => /^(?:relabel|re-label|fix\s+labels?|infer\s+labels?)\s*(?:all|edges?)?\s*$/i.test(s), 'relabel', 'high');
...
// Compress
addPattern((s) => /^(?:compress|compact|simplify|dedupe|dedup)(?:\s+(?:the\s+)?(?:workflow|graph|project|all|nodes))?\s*$/i.test(s), 'compress', 'high');

// Bottlenecks
addPattern((s) => /^(?:bottleneck|bottlenecks|choke|chokepoint|hub|hubs|spof)\s*$/i.test(s), 'bottlenecks', 'high');

// Suggest
addPattern((s) => /^(?:suggest|next|what\s*(?:should|can)\s*I\s*do(?:\s+next|\s+now)?|recommendations?)\s*$/i.test(s), 'suggest', 'medium');
addPattern((s) => /^(?:which|what)\s+nodes?\s+(?:need|require|want)\s+(?:attention|work|updating|fixing|help)/i.test(s), 'suggest', 'medium');
addPattern((s) => /^(?:which|what)\s+nodes?\s+(?:haven't|have\s+not|aren't|are\s+not)\s+been\s+(?:updated|changed|run|executed)/i.test(s), 'suggest', 'medium');

// Auto-describe
addPattern((s) => /^(?:auto[- ]?describe|describe\s+all|fill\s+descriptions?)(?:\s+(?:all\s+)?(?:nodes?|empty)?)?\s*$/i.test(s), 'auto-describe', 'high');

// Refine
addPattern((s) => /^(?:refine|extract|structure)(?:\s+(?:this\s+)?note)?\s*$/i.test(s), 'refine', 'high');

// ── Central Brain Commands ──
addPattern((s) => /^(?:ingest|feed|analyze|here(?:'s| is) (?:my|the|some)|take this|source(?:\s*material)?:)/i.test(s), 'ingest', 'medium');
addPattern((s) => /^(?:understand(?:ing)?|what do you (?:know|understand)|show (?:me )?(?:your )?(?:understanding|context|source)|context)\s*$/i.test(s), 'understand', 'medium');
addPattern((s) => /^(?:create|generate|build|make|write)\s+(?:a\s+)?(?:new\s+)?(?:blog[\s-]?post|email|social[\s-]?(?:thread|post|media)|twitter[\s-]?thread|x[\s-]?thread|ad[\s-]?copy|press[\s-]?release|landing[\s-]?page|newsletter|product[\s-]?description|linkedin[\s-]?post|ph[\s-]?(?:tagline|copy)|pitch|summary|brief|article|copy)/i.test(s), 'create-artifact', 'medium');
addPattern((s) => /^(?:sync|sync all|sync stale|sync everything|resync|re-sync)\s*$/i.test(s), 'sync-artifacts', 'medium');
addPattern((s) => /^sync\s+["']?.+["']?\s*$/i.test(s), 'sync-artifacts', 'medium');
addPattern((s) => /^(?:diff|preview sync|what(?:'s| would) (?:change|sync)|stale artifacts)\s*$/i.test(s), 'diff-artifacts', 'medium');
addPattern((s) => /^(?:overrides?|show overrides?|list overrides?|my (?:edits|changes|overrides))\s*$/i.test(s), 'show-overrides', 'high');
addPattern((s) => /^(?:forget|remove|clear)\s+override\b/i.test(s), 'forget-override', 'high');
addPattern((s) => /^(?:update|change|edit|modify)\s+(?:the\s+)?(?:source|input|original|context)\b/i.test(s), 'update-source', 'medium');
